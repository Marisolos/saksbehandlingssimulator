    ...
    if not cases:
        ...
        for c in filtered_cases:
            ...
            rows.append({
                ...
                "Navn": a.name,
                "Alder": a.age,
                "Inntekt": f"{a.income:,.0f} kr",
                "Arbeidsstatus": a.employment_status,
                "Barn": "Ja" if a.has_children else "Nei",
                "Utdanning": {
                    "none": "Ingen utdanning",
                    "highschool": "Videregående",
                    "university": "Høyere utdanning"
                }.get(a.education_level, "?"),
                "Dokumentasjon": "Levert" if a.documentation_provided else "Ikke levert",
                "Status": "✅ Innvilget" if c.status == "approved" else "❌ Avslått",
                "Begrunnelse": c.decision_reason,
                "Dato": c.created_at.strftime("%d.%m.%Y %H:%M") if c.created_at else "?"
            })
        ...
        df_stats = pd.DataFrame.from_dict(counter, orient='index', columns=['Antall saker'])
        ...
        if st.button("📅 Last ned filtrerte saker som CSV"):
            ...
            writer.writerow([
                "SakID", "Navn", "Alder", "Inntekt", "Arbeidsstatus", "Barn",
                "Utdanning", "Dokumentasjon", "Status", "Begrunnelse", "Dato"
            ])
            for c in filtered_cases:
                a = session.query(Applicant).get(c.applicant_id)
                writer.writerow([
